...
class RestrainingBoltAgent(Agent):
    def __init__(self, env, dist=False, specs=None, alpha=0.5, epsilon=0.05, gamma=0.9, ntrain=10, export=''):
    ...
    def test(self, rec=False):
        ...
        while not state.final:
            ...
            for i in range(len(aStates)):
                inpt = self.get_labels(nstate) + [act]

                aStates[i] = self.automata[i].transition(aStates[i], inpt)
                if aStates[i] in self.automata[i].final:
                    r = r + self.automata[i].reward
                if aStates[i] in self.automata[i].sinkStates:
                    aStates[i] = 0
            steps += 1
    ...
    def train(self):
        for n in range(self.ntrain):
            ...
            while not state.final:
                ...
                old_aStates = []
                for i in range(len(aStates)):
                    old_aStates = aStates.copy()
                    aStates[i] = self.automata[i].transition(aStates[i], inpt)
                    if aStates[i] in self.automata[i].final:
                        r += self.automata[i].reward
                    if aStates[i] in self.automata[i].sinkStates:
                        aStates[i] = 0
                self.update(state, act, nstate, old_aStates, aStates, r)
                reward += r
                state = nstate
...
class MORLRBNormAgent(Agent):
    def __init__(self, env, dist=False, specs=None, alpha=0.5, epsilon=0.05, gamma=0.9, ntrain=10, export='', plot=False, seed=None):
        Agent.__init__(self, env, dist)
    ...
    def train(self):
        ...
        for n in range(self.ntrain):
            ...
            while not state.final:
                ...
                #print(rwd)
                #print(self.getQValueVector(state,act,oldaStates))
        ...
        if self.plot:
            ...
            for i in range(len(self.weights)):
                plot = []
                for j in range(int(self.ntrain / 20)):
                    avg = average(allrw[i][j * 20:(j + 1) * 20])
                    print(i, ',', j)
                    print(avg)
                    plot.append(avg)
                plots.append(plot)
            print(plots)
            x = range(int(self.ntrain / 20))
            xs = [(p + 1) * 20 for p in x]
            for i in range(len(self.weights)):
                plt.plot(xs, plots[i])
            plt.show()


class MORLRBAgent(Agent):
    def __init__(self, env, dist=False, specs=None, alpha=0.5, epsilon=0.05, gamma=0.9, ntrain=10, export=''):
        Agent.__init__(self, env, dist)
        self.name = 'MORL Restraining Bolt Agent'
        self.logger = log.Log(self.name, self.env.map)
        self.qValues = {}
        self.qValues = defaultdict(lambda:0.0, self.qValues)
        self.alpha = alpha
    ...
    def policy(self, state, aStates, train):
        possible = self.getLegalActions(state, train)
        val = self.computeValue(state, possible, aStates)
        acts = []
        for act in possible:
            q = 0
            for i in range(len(self.allQValues)):
...
class MORLRBNormLexAgent(Agent):
    def __init__(self, env, dist=False, specs=None, alpha=0.5, epsilon=0.05, gamma=0.9, ntrain=10, export='', plot=False, seed=None):
        Agent.__init__(self, env, dist)
        self.name = 'MORL Restraining Bolt Norm Agent'
        self.logger = log.Log(self.name, self.env.map)
